...
const URL: LazyCell<String> =
    LazyCell::new(|| var("DATABASE_URL").expect("Make sure to set DATABASE_URL in .env"));
...
async fn main() {
    ...
    let app: Router = route(pool).layer(from_fn(handle_request));
...
async fn handle_request(req: Request<Body>, next: Next) -> Result<Response, Infallible> {
